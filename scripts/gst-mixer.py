...
class Client:
    ...
    def link_streams(self,prefix,qparams):
        ...
        for c in clients:
            ...
            if not prefix+self.ssrc+"_"+c in mixer_links:
                print("  linking client "+self.ssrc+" to "+prefix+"mixer "+c)
                add_and_link([
                    getattr(self,prefix+"tee"),
                    new_element("queue",qparams),
                    getattr(other,prefix+"mixer")
                ])
                mixer_links.append(prefix+self.ssrc+"_"+c)

            # for every _other_ tee, link that tee to my mixer
            if not prefix+c+"_"+self.ssrc in mixer_links:
                print("  linking client "+c+" to "+prefix+"mixer "+self.ssrc)
                add_and_link([
                    getattr(other,prefix+"tee"),
                    new_element("queue",qparams),
                    getattr(self,prefix+"mixer")
                ])
                mixer_links.append(prefix+c+"_"+self.ssrc)
