...
audioenc="$queue ! opusenc bitrate=16000 ! queue max-size-time=100000000"
...
command="                    ! videorate ! video/x-raw,width=1280,height=720,framerate=10/1 ! $videoenc ! mux. \
  ...
  mpegtsmux name=mux alignment=7 ! tee name=fork ! rtpmp2tpay ! udpsink ${BIND_ADDRESS} host=${1:-127.0.0.1} port=5000 $record"
